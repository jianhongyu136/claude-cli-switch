    ...
    async fn forward(
        ...
        let mut auth_headers = if let Some(mut auth) = adapter.extract_auth(provider) {
            ...
            if auth.strategy == AuthStrategy::GitHubCopilot {
                ...
                {
                    return Err(ProxyError::AuthError(
                        "GitHub Copilot managed-account auth is unavailable in standalone ccs because it requires GUI OAuth state".to_string(),
                    ));
                }
            }
            ...
            if auth.strategy == AuthStrategy::CodexOAuth {
                ...
                {
                    return Err(ProxyError::AuthError(
                        "Codex OAuth managed-account auth is unavailable in standalone ccs because it requires GUI OAuth state".to_string(),
                    ));
                }
            }
